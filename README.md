...
| 폴더 | 설명 |
| --- | --- |
| `CG_Homework01` | 무작위 색상·속도의 사각형들을 생성하고, 화면 경계 및 사각형 간 충돌을 처리합니다. |
| `CG_Homework02` | 정점·색상 배열과 `GL_QUADS`로 컬러 큐브를 만들고 깊이 테스트와 후면 컬링을 적용합니다. 방향키로 회전할 수 있습니다. |
| `CG_Homework03` | LodePNG로 세 행성의 텍스처를 읽고 공전 애니메이션을 렌더링합니다. |
...
| `CG_Practice02_BounceRect` | 사각형 하나를 화면 경계에서 바운스합니다. |
...
| `CG_Practice07_Light` | 조명, 재질, 깊이 테스트와 컬링을 적용한 제트 모델입니다. 방향키로 회전하고 `F1`/`F2`로 거리를 조절합니다. |
| `CG_Practice08_Texture` | LodePNG로 `텍스처를 읽어 주전자에 매핑하고 조명, 래핑, 선형 필터링을 적용합니다. 방향키로 회전하고, `W`/`S` 거리를 조절합니다. |
